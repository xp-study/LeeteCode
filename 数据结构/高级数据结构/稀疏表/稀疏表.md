...
查询的原理上面已经说得很清楚了，现在的问题是如何找到那个不大也不小的小区间使得这个区间不能覆盖大区间，但是这个区间的两倍要覆盖大区间。我们把这个区间的长度写成二进制，比如**len=7(1011)2**，我们每次去二进制中最高一位的1就可以满足这个要求了，**(1000)2<(1011)2**且两倍**1000**等于 **(1000)2<<1=(10000)>(1011)2**。
